...
import { formatMessageTime, getAttachment, type SampleMessage } from "../data/messages";
...
export function MessageList({
  ...
  const folderLabel = FOLDERS.find((candidate) => candidate.id === folder)?.label ?? folder;
  ...
  const visibleMessages = sortMessages(filterMessages(allMessages, filters), sortKey);
  const activeFilters = hasActiveFilters(filters);
  const currentSortLabel = SORT_OPTIONS.find((option) => option.key === sortKey)?.label ?? "";

  function renderItem({ item, index }: { item: SampleMessage; index: number }) {
    ...
    return (
      <View style={styles.rowWrapper}>
        ...
        <Pressable onPress={() => onToggleStar(item.id)} style={styles.starButton}>
    ...
    <View style={[styles.pane, { width }]}>
      <View style={styles.header}>
        <View style={styles.headerTop}>
          <Text style={styles.title}>{folderLabel}</Text>
          {/* Explicit zIndex here (not just on `header`) -- this is what
              makes the floating menu below paint in front of searchRow,
              rather than searchRow's own box winning the stacking order. */}
          <View style={styles.sortWrapper}>
            <Pressable onPress={() => setSortMenuOpen((value) => !value)} style={styles.sortButton}>
              ...
              <View style={styles.sortMenu}>
                {SORT_OPTIONS.map((option) => {
                  ...
                  return (
                    ...
                    >
                      <Text style={[styles.sortMenuItemText, active && { color: accentColor }]}>{option.label}</Text>
                    </Pressable>
                  );
                })}
              </View>
        ...
        <View style={styles.searchRowWrapper}>
          ...
          {filtersOpen && (
          // Floats over the message list below -- same reasoning as the
          // sort menu, this must not push list rows down.
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    // stacking context otherwise, so the sort menu's zIndex only resolves
    // against its own siblings inside header, not against the overlay.
    zIndex: 15,
  },
  headerTop: {
    flexDirection: "row",
  ...
  sortMenu: {
    ...
    // Floats in front of searchRow below -- sortWrapper's own zIndex is
    // what wins the comparison against searchRow's stacking, this one only
    // has to beat sortButton, its one sibling inside sortWrapper.
    zIndex: 1,
  },
  sortMenuItem: {
