...
- [x] Local encrypted cache schema — SQLCipher-encrypted SQLite (`rusqlite`) caching message summaries and bodies as they're fetched over IMAP, key managed via the OS keychain (`src-tauri/src/cache.rs`) — see `local-cache.md`. Now also home to the `accounts` and `contacts` tables below. Offline reads now exist (see the offline-read item below); still no attachment-bytes, draft-queue, or FTS tables — those remain unimplemented.
- [x] Offline reads — `cache::load_cached_messages`/`load_cached_message_body` read the last-cached summaries/body back out without touching the network, for display when a live fetch can't reach the server. `cached_messages` gained `message_id`/`in_reply_to` columns (additive migration) so offline summaries can still be threaded. A cached body carries only text/html + threading IDs (no attachment bytes/recipients/PGP state). The backend doesn't auto-fall-back on a failed `fetch_messages` — that's left to the frontend so it can flag staleness — see `local-cache.md`.
- [x] IMAP server-side search — `imap::search_messages` runs `UID SEARCH` matching a free-text query as a substring of Subject/From/To/Body, returns summaries for the newest `limit` matches. Query is escaped against IMAP command injection (CR/LF stripped, quotes/backslashes escaped) and gets a `CHARSET UTF-8` prefix only when non-ASCII. Single-folder, free-text only — no structured/date/flag search, no cross-folder search, no offline/FTS search — see `imap-search.md`.
- [x] Folder operations — `imap::create_folder`/`delete_folder`/`rename_folder`/`empty_folder` (CREATE/DELETE/RENAME, and a `1:*`-store+EXPUNGE "empty trash"). No `SPECIAL-USE` resolution or subscription management — see `folder-operations.md`.
- [x] Batch/multi-select mutations — `imap::set_messages_seen`/`set_messages_flagged`/`move_messages_to_folder` (UID-set variants of the single-message commands, one round-trip per selection) and `mark_folder_seen` (the "mark all as read" `1:*` store). The single-message commands now delegate to the same UID-set-aware internal helpers — see `mailbox-actions.md`.
- [x] IMAP STARTTLS — `connect_and_login` gained a `use_starttls` flag (STARTTLS on 143 vs. implicit TLS on 993), the mandatory-upgrade/no-plaintext-fallback posture matching SMTP/POP3. `AccountRecord.imap_use_starttls` persists it; data commands resolve it from the cached account via `login_for_account` (no signature change), onboarding passes it explicitly via `verify_and_list_folders`. Frontend onboarding has an "IMAP uses STARTTLS" advanced toggle — see `imap-core.md`.
- [x] POP3 account-model integration — `AccountRecord` gained `incoming_protocol`/`pop3_host`/`pop3_port`; `account::add_pop3_account` onboards and verifies a POP3 account (`pop3::verify_login`); `fetch_unified_inbox` skips POP3 accounts. POP3 mail commands still take host/port directly, and POP3 mail isn't in the unified view — see `pop3.md`/`multi-account.md`.
- [x] Account update command — `account::update_account` rotates a password (optional) and/or changes host/port without remove+re-add, re-verifying before commit (over IMAP or POP3 per protocol) and rolling a failed password change back to the previous one — see `multi-account.md`.
- [x] Contact list/delete — `cache::list_contacts` (whole address book, most-recent first) and `cache::delete_contact` (forget a harvested address), complementing the autocomplete-only `search_contacts` — see `contacts.md`.
- [x] Draft attachments — `save_draft` takes `attachments`, stored on the draft row as JSON and APPENDed as a hand-built `multipart/mixed` message (`build_raw_draft_bytes`); previously drafts were text-only — see `drafts.md`.
